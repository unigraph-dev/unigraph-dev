  ...
  const eventRouter: Record<string, EventResponser> = {
    ...
    "subscribe_to_type": function (event: EventSubscribeType, ws: IWebsocket) {
      lock.acquire('caches/schema', function(done: (any)) {
        ...
        const query = event.schema === "any" ? (event.all ? queryAnyAll : queryAny) : `(func: uid(par${event.id})) @filter((type(Entity)) AND (NOT eq(<_propertyType>, "inheritance")) AND (NOT eq(<_hide>, true))  AND (NOT type(Deleted)))

...
export default async function startServer(client: DgraphClient) {
    ...
    const eventRouter: Record<string, EventResponser> = {
            ...
            lock.acquire('caches/schema', function (done: Function) {
                ...
                if (names.includes(event.packageName)) {
                    ...
                    addUnigraphPackage(dgraphClient, eventb.package, caches)
                        ...
                        .catch((e) => {
                            ws.send(makeResponse(eventb, false, { error: e.toString() }));
                            done(true, null);
                        });
                }
            });
        ...
        delete_unigraph_object: async function (event: EventDeleteUnigraphObject, ws: IWebsocket) {
            await localApi
                .deleteObject(event.uid, event.permanent)
                .catch((e: any) => ws.send(makeResponse(event, false, { error: e })));
        },
        ...
        create_unigraph_object: function (event: EventCreateUnigraphObject, ws: IWebsocket) {
            if (!event.schema) {
                ws.send(makeResponse(event, false, { error: 'Cannot add Unigraph object without a schema!' }));
                return false;
            }
            localApi
        ...
        update_spo: function (event: EventUpdateSPO, ws: IWebsocket) {
            localApi
                .updateTriplets(event.objects, event.isDelete, event.subIds)
                .then((_: any) => {
                    callHooks(serverStates.hooks, 'after_object_changed', {
                        subscriptions: serverStates.subscriptions,
                        caches: caches,
        ...
        update_object: async function (event: EventUpdateObject, ws: IWebsocket) {
            ...
            let origin;
            if (!event.origin) {
                origin = event.newObject['unigraph.origin']
                    ? event.newObject['unigraph.origin']
                    : (
                          await dgraphClient.queryData<any>(
            ...
            if (typeof event.upsert === 'boolean' && !event.upsert) {
                if (event.pad) {
                    ws.send(
                        makeResponse(event, false, {
                            error: 'In non-upsert mode, you have to supply a padded object, since we are mutating metadata explicitly as well.',
                        }),
                    );
                } else {
                ...
                if (event.pad !== false) {
                    ...
                    const updater = {
                        ...event.newObject,
                        uid: event.uid ? event.uid : event.newObject.uid,
        ...
        delete_relation: async function (event: EventDeleteRelation, ws: IWebsocket) {
            localApi
                .deleteRelation(event.uid, event.relation)
                .then(() => {
                    ws.send(makeResponse(event, true));
                })
                .catch((e: any) => ws.send(makeResponse(event, false, { error: e.toString() })));
        ...
        reorder_item_in_array: async function (event: EventReorderItemInArray, ws: IWebsocket) {
            (localApi as any)
                .reorderItemInArray(event.uid, event.item, event.relUid, event.subIds)
                .then((_: any) => {
                    ws.send(makeResponse(event, true));
                })
                .catch((e: any) => ws.send(makeResponse(event, false, { error: e.toString() })));
        ...
        delete_item_from_array: async function (event: EventDeleteItemFromArray, ws: IWebsocket) {
            localApi
                .deleteItemFromArray(event.uid, event.item, event.relUid, event.subIds)
                .then((_: any) => {
                    ws.send(makeResponse(event, true));
                })
                .catch((e: any) => ws.send(makeResponse(event, false, { error: e.toString() })));
        ...
        hibernate_or_revive_subscription: async function (event: EventHibernateSubscription, ws: IWebsocket) {
            const res = await serverStates.localApi
                .hibernateOrReviveSubscription(event.ids || event.id, event.revival)
                .catch((e: any) => ws.send(makeResponse(event, false, { error: e.toString() })));
            ws.send(makeResponse(event, true, { result: res }));
        },
        ...
        import_objects: async function (event: EventImportObjects, ws: IWebsocket) {
            ...
            const dectx: any[] = dectx_raw.filter(
                (el) => !(el['dgraph.type']?.includes('Executable') || el['dgraph.type']?.includes('Named')),
            );
            const ref = dectx
                .map((el, index) => {
                    try {
        ...
        get_search_results: async function (event: EventGetSearchResults, ws: IWebsocket) {
            ...
            serverStates.localApi
                .getSearchResults(searchQuery, event.display, event.hops, searchOptions)
        ...
        export_objects: async function (event: EventExportObjects, ws: IWebsocket) {
            const res = await serverStates.localApi
                .exportObjects(event.uids, event.options)
                .catch((e: any) => ws.send(makeResponse(event, false, { error: e.toString() })));
            ws.send(makeResponse(event, true, { result: res }));
        },
        ...
        disable_package: async function (event: EventDisablePackage, ws: IWebsocket) {
            const res = await disablePackage(client, serverStates, event.packageName).catch((e: any) =>
                ws.send(makeResponse(event, false, { error: e.toString() })),
            );
            callHooks(serverStates.hooks, 'after_object_changed', {
                subscriptions: serverStates.subscriptions,
        ...
        enable_package: async function (event: EventEnablePackage, ws: IWebsocket) {
            const res = await enablePackage(client, serverStates, event.packageName).catch((e: any) =>
                ws.send(makeResponse(event, false, { error: e.toString() })),
            );
            callHooks(serverStates.hooks, 'after_object_changed', {
                subscriptions: serverStates.subscriptions,
        ...
        recalculate_backlinks: async function (event: EventRecalculateBacklinks, ws: IWebsocket) {
            const res = await serverStates.localApi
                .recalculateBacklinks(event.fromUids, event.toUids, event.depth)
                .catch((e: any) => ws.send(makeResponse(event, false, { error: e.toString() })));
            ws.send(makeResponse(event, true, { result: res }));
        },
        ...
        add_backlinks: async function (event: EventAddBacklinks, ws: IWebsocket) {
            const res = await serverStates.localApi
                .addBacklinks(event.fromUids, event.toUids)
                .catch((e: any) => ws.send(makeResponse(event, false, { error: e.toString() })));
            ws.send(makeResponse(event, true, { result: res }));
        },
    ...
    initEntityHeads(
        serverStates,
        Object.keys(serverStates.caches['schemas'].data).filter((el) => el.startsWith('$/schema/')),
        client,
    );
    ...
    serverStates.getClientId = (connId: string) =>
        Object.entries(historialClients).filter((el) => el[1] === connId)?.[0]?.[0] || getRandomInt();
    ...
    server.on('connection', (ws, req) => {
        ...
        if (
            ...
            if (Object.keys(historialClients).includes(clientBrowserId)) {
                serverStates.subscriptions = removeOrHibernateSubscriptionsById(
                    serverStates.subscriptions,
                    historialClients[clientBrowserId],
                    undefined,
                );
            }
        ...
        if (revival) {
            serverStates.subscriptions = reviveSubscriptions(
                serverStates.subscriptions,
                historialClients[clientBrowserId],
                clientBrowserId,
                ws,

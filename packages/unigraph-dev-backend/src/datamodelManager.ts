...
import {
    buildGraphFromMap, buildUnigraphEntity, processAutoref, processAutorefUnigraphId, unpad,
} from 'unigraph-dev-common/lib/utils/entityUtils';
import { getRefQueryUnigraphId } from 'unigraph-dev-common/lib/utils/utils';
import { Cache } from './caches';
import {
    defaultPackages, defaultTypes, defaultUserlandSchemas, packageManifestSchema,
    ...
    // Insert default data
        ...
        for (let i = 0; i < defaultUserlandSchemas.length; i += 1) {
            ...
            await client.createUnigraphUpsert(
                insertsToUpsert([schema], undefined, tempSchemaCache.dataAlt![0]),
            );
        }
    ...
    cache.updateNow = async () => {
        ...
        newdata2.forEach((obj: any) => {
            if (obj
                ...
                if (
                    cache.data[`$/schema/interface/${revPath[0]}`]?._definition?._parameters?._definitions?.length >= 0
...
export async function addUnigraphPackage(
    ...
    const entities = !(pkg.pkgEntities && Object.entries(pkg.pkgEntities))
        ? []
        : Object.entries(pkg.pkgEntities).map(([key, obj]: any) => {
    ...
    const pkgObj = {
        ...
        pkgExecutables: !(pkg.pkgExecutables && Object.entries(pkg.pkgExecutables))
            ? undefined
            : Object.fromEntries(executables.map((exec, i) => [
        ...
        pkgEntities: !(pkg.pkgEntities && Object.entries(pkg.pkgEntities))
            ? undefined
            : Object.fromEntries(entities.map((et, i) => [

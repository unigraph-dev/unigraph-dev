...
export function getFragment(query: Query, states: any) {
    if (query.type === 'query') {
        ...
        const frag = (query.options?.noExpand || query.fragment.startsWith('$/executable/'))
            ? query.fragment
        ...
        const {
            all, showHidden, uidsOnly, first, metadataOnly, depth, queryAs,
        ...
        const frag = query.name === 'any'
            ? ((all || uidsOnly) ? queryAnyAll : queryAny)
            ...
            if (el && !el.hibernated) {
                ...
                if (getFragment(el.query, serverStates).startsWith('$/executable/')) {
                    ...
                    query = buildPollingQuery(
                        [{ ...el, query: { ...el.query, type: 'query', fragment: executed } as any }],
                        serverStates,
                    );
                ...
                const queryFn = async () => {
                    ...
                    const results: any[] = await client.queryDgraph(query)
                        .catch((e) => { console.log(e, query); return []; });
                    const val = results[0];

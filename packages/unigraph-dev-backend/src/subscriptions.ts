...
export function getFragment(query: Query, states: any) {
    if (query.type === 'query') {
        ...
        const frag =
            query.options?.noExpand ||
            query.fragment.startsWith('$/executable/')
                ? query.fragment
    ...
    if (query.type === 'type') {
        ...
        ${
            uidsOnly
                ...
                : metadataOnly
                ? ' { uid <dgraph.type> type { <unigraph.id> } _updatedAt } '
                : queryAs ||
    ...
    if (query.type === 'object') {
        ...
        const frag = options.queryFn
            ? options.queryFn.replace(
                  'QUERYFN_TEMPLATE',
                  Array.isArray(uid) ? uid.join(', ') : uid,
              )
...
export async function pollSubscriptions(
    ...
    if (subs.length >= 1) {
        subs.map((el) => (ids?.includes(el.id) ? el : false)).forEach(
            async (el, index) => {
                if (el && !el.hibernated) {
                    let query: string;
                    if (
                        getFragment(el.query, serverStates).startsWith(
                            '$/executable/',
                        )
                    ) {
                        const exec =
                            serverStates.caches.executables.data[
                                getFragment(el.query, serverStates)
                            ];
...
export function reviveSubscriptions(
    ...
    return subscriptions.map((el) =>
        el.connId === connId ? { ...el, ...newData } : el,
    );
}
...
const resolvers = {
    ...
    object: {
        match: (oldQuery: QueryObject, newQuery: QueryObject) =>
            ...
            (!newQuery.options ||
                _.isEqual(oldQuery.options, newQuery.options)),
        resolve: async (
            ...
            const newUid = Array.isArray(newQuery.uid)
                ? newQuery.uid
                : [newQuery.uid];
            const oldUid = Array.isArray(oldQuery.uid)
                ? oldQuery.uid
                : [oldQuery.uid];
            const addUids = _.difference(newUid, oldUid);
            let newData = sub.data;
            if (addUids.length !== 0) {
                const query = `{ sub${getFragment(
                    { ...newQuery, uid: addUids },
                    states,
                )} }`;
                const results: any[] = await states.dgraphClient
                    .queryDgraph(query)
                    .catch((e: any) => {
                        console.log(e, query);
                        return [];
...
export function resolveSubscriptionUpdate(
    subsId: number | string,
    newQuery: Query,
    states: any,
) {
    const sub = states.subscriptions.filter(
        (el: Subscription) => el.id === subsId,
    )?.[0];
    ...
    return (resolvers[newQuery.type] as any).resolve(
        oldQuery,
        newQuery,
        sub,
        states,

...
export function getLocalUnigraphAPI(
    ...
    const api: Unigraph = {
        ...
        subscribeToType: async (name, callback: any, eventId = undefined, options: any) => api.subscribe({ type: 'type', name, options }, callback, eventId),
        subscribeToObject: async (uid, callback: any, eventId = undefined, options: any) => api.subscribe({ type: 'object', uid, options }, callback, eventId),
        subscribeToQuery: async (fragment, callback: any, eventId = undefined, options: any) => api.subscribe({ type: 'query', fragment, options }, callback, eventId),
        ...
        getQueries: async (fragments, getAll = false, batch = 50, commonVars) => {
            ...
            if (getAll) {
                allQueries = fragments.map((it, index) => `query${index}(func: uid(par${index})) @recurse {uid unigraph.id expand(_userpredicate_)}
            par${index} as var${it}`);
            ...
            try {
                if (pad) {
                    ...
                    const paddedUpdater = buildUnigraphEntity(newObject, schema, states.caches.schemas.data, true, {
                        validateSchema: true, isUpdate: true, states: {}, globalStates: {},
                    });
        ...
        getSchemas: async (schemas: string[] | undefined, resolve = false) => Object.fromEntries(Object.entries(states.caches.schemas.data).filter(([k, _]) => !schemas?.length || schemas?.includes(k))) as any,
        ...
        runExecutable: async (uid: string, params: any, context?: ExecContext) => {
            ...
            await states.lock.acquire('caches/exec', async (done: any) => {
                ...
                const execFn = await buildExecutable(exec, { params, definition: exec, ...context }, states.localApi, states);
        ...
        awaitHttpCallback: (key: string) => new Promise((resolve, _) => {
        ...
        getSubscriptions: () => states?.subscriptions?.map((el) => ({

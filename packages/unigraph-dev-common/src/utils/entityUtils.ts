...
export function makeUnigraphId<IdType extends string>(id: IdType): UnigraphIdType<IdType> {
    return { 'unigraph.id': id };
}
export function makeRefUnigraphId<IdType extends string>(id: IdType): RefUnigraphIdType<IdType> {
    return { $ref: { query: [{ key: 'unigraph.id', value: id }] } };
}
...
function isUnion(schemaString: string, schemaMap: Record<string, any>): boolean {
    ...
    return (
        schemaString.startsWith('$/schema/interface') ||
        schemaString === '$/composer/Union' ||
        isUnion(schemaMap[schemaString]?.type?.['unigraph.id'], schemaMap)
    );
}
...
function isRef(rawPart: any) {
    return (
        ...
        (Object.keys(rawPart).length === 1 &&
            typeof rawPart['unigraph.id'] === 'string' &&
            rawPart['unigraph.id'].startsWith('$/'))
    );
}
 ...
 */
function buildUnigraphEntityPart(
    rawPart: any,
    options: BuildEntityOptions,
    schemaMap: Record<string, Schema>,
    localSchema: Definition | any,
    propDesc: PropertyDescription | Record<string, never> = {},
): { _value: any } {
    ...
    if (
        ...
        schemaMap[rawPart.type['unigraph.id']]?._definition &&
        (rawPart?.type?.['unigraph.id'] === localSchema.type?.['unigraph.id'] ||
            localSchema.type?.['unigraph.id'] === '$/schema/any')
    ) {
        const userType = rawPart.type;
        delete rawPart.type;
        unigraphPartValue = buildUnigraphEntity(
            rawPart._value || rawPart._value === '' ? rawPart._value : rawPart,
            userType['unigraph.id'],
            schemaMap,
            true,
            options,
            propDesc,
        ...
        try {
            ...
            if (rawPart && isRef(rawPart)) {
                ...
                switch (rawPartUnigraphType) {
                    ...
                    case '$/composer/List':
                        ...
                        unigraphPartValue = rawPart.map((el: any) =>
                            buildUnigraphEntityPart(el, options, schemaMap, newLocalSchema2),
                        );
                        break;
                    ...
                    case '$/composer/Object':
                        ...
                        Object.entries(rawPart).forEach(([key, value]: [string, any]) => {
                            ...
                            const propDesc: PropertyDescription = _.pickBy(
                                { _propertyType: keysMap[key]._propertyType },
                                _.identity,
                            );
                            if (!localSchema)
                                throw new TypeError('Schema check failure for object: ' + JSON.stringify(rawPart));
                            unigraphPartValue[key] = {
                                ...buildUnigraphEntityPart(value, options, schemaMap, localSchema, propDesc),
                                ...context,
                            };
                            if (typeof keysMap[key]?.['_indexAs'] === 'string') {
                                const linkUid =
                                    unigraphPartValue[key]['uid'] || '_:link' + options.globalStates.nextUid++;
                ...
                localSchema.type &&
                isTypeAlias(schemaMap[localSchema.type['unigraph.id']]?._definition, rawPartUnigraphType)
            ) {
                ...
                if (rawPart?.type?.['unigraph.id']) {
                    const userType = rawPart.type;
                    if (
                        Object.keys(rawPart).length === 2 &&
                        typeof rawPart.uid === 'string' &&
                        rawPart.uid.startsWith('0x')
                    )
                        delete rawPart.type;
                    definitions = unionSchema._parameters._definitions.filter(
                        (el: any) => el.type['unigraph.id'] === userType['unigraph.id'],
                    );
                    //rawPart = (rawPart['_value'] || rawPart['_value'] === '') ? rawPart['_value'] : rawPart;
                }
                let choicesResults = _.uniqBy(definitions, 'uid')
                    .map((defn) => {
                        try {
            ...
            console.log('buildUnigraphEntityPart', {
                localSchema,
                localSchemaProps: localSchema?._properties,
                rawPart,
                rawPartUnigraphType,
            });
            throw new Error(
                'Building entity part error: ' +
                    e +
                    '\n' +
                    e.stack +
                    '\n' +
 ...
 */
export function buildUnigraphEntity(
    raw: Record<string, any>,
    schemaName = 'any',
    schemaMap: Record<string, Schema>,
    padding = true,
    options: BuildEntityOptions = { validateSchema: true, isUpdate: false, states: {}, globalStates: {} },
    propDesc: any = {},
): EntityDgraph<string> | TypeError {
    // initialize states
    ...
    if (padding === false && !validatePaddedEntity(raw)) {
        ...
        const bodyObject: Record<string, any> = padding
            ? buildUnigraphEntityPart(raw, options, schemaMap, localSchema, {})
            : raw;
...
export function makeQueryFragmentFromType(
    schemaName: string,
...
function unpadValue(something: any) {
    ...
    kvs.forEach(([key, value]) => {
        if (key.startsWith('_value')) ret = value;
    });
    return ret;
}

function getKey(object: any, key: string) {
    return object['_value'][key];
}
...
export function processAutoref(entity: any, schema = 'any', schemas: Record<string, Schema>, unigraphIdReplacer?: any) {
    ...
    function recurse(currentEntity: any, schemas: Record<string, Schema>, localSchema: Definition | any) {
        ...
        if (
            currentEntity?.type?.['unigraph.id'] &&
            currentEntity.type['unigraph.id'] !== paddedEntity?.type?.['unigraph.id']
        )
        ...
        switch (typeof currentEntity) {
            case 'object':
                ...
                if (Array.isArray(currentEntity) && localSchema['type']?.['unigraph.id'] === '$/composer/Array') {
                    currentEntity.forEach((e) =>
                        recurse(unpadValue(e), schemas, localSchema['_parameters']['_element']),
                    );
                } else if (
                    localSchema?.type?.['unigraph.id'] === '$/composer/Object' ||
                    Object.keys(currentEntity).includes('unigraph.id')
                ) {
                    ...
                    const keysMap =
                        localSchema?.['_properties']?.reduce((accu: any, now: any) => {
                            accu[now['_key']] = now;
                            return accu;
                    ...
                    kv.forEach(([key, value]) => {
                        if (key === 'unigraph.id') {
                            ...
                            if (localSchema['_unique']) {
                                ...
                                paddedEntity['$ref'] = {
                                    query: [
                                        { key: key, value: unpadValue(value) },
                            ...
                            try {
                                recurse(
                                    unpadValue(value),
                                    schemas,
                                    schemas[(value as any)?.type?.['unigraph.id']]['_definition'],
                                );
                            } catch (e) {
                                console.error(value);
                                throw e;
...
export function getUpsertFromUpdater(orig: any, updater: any): any {
    function recurse(origNow: any, updaterNow: any): any {
        if (['undefined', 'null', 'number', 'bigint', 'string', 'boolean', 'symbol'].includes(typeof origNow)) {
            ...
            return updaterNow.map((value, index) =>
                index < origNow.length ? recurse(origNow[index], value) : recurse(undefined, value),
            );
        } else if (typeof origNow == 'object' && !Array.isArray(origNow) && !Array.isArray(updaterNow)) {
...
export const byUpdatedAt = (a: any, b: any) =>
    new Date(a?.['_updatedAt'] || 0).getTime() - new Date(b?.['_updatedAt'] || 0).getTime();
...
export function unpadRecurse(object: any, visitedUids: any[] = []) {
    ...
    if (typeof object === 'object' && !Array.isArray(object)) {
        ...
        if (predicate) {
            // In simple settings, if contains _value ignore all edge annotations
        ...
        if (timestamp_c && typeof result === 'object' && !Array.isArray(result))
            result['_createdAt'] = object['_createdAt'];
        if (timestamp_u && typeof result === 'object' && !Array.isArray(result))
            result['_updatedAt'] = object['_updatedAt'];
...
export function isPaddedObject(obj: any) {
    ...
    Object.keys(obj).forEach((key) => {
        if (key.startsWith('_value')) valueKeysCount++;
    });
...
export function flatten(objs: any[]) {
    ...
    function flattenRecurse(curr: any) {
        if (typeof curr === 'object' && curr && Array.isArray(curr)) {
            ...
            if (
                curr.uid &&
                (curr['dgraph.type']?.includes?.('Entity') || curr['dgraph.type']?.includes?.('Interface'))
            ) {
                // Flatten entities
                if (!entities[curr.uid]) entities[curr.uid] = curr;
                else
                    Object.keys(curr).forEach((el) => {
                        if (el !== 'uid') delete curr[el];
                    });
            }
            // Remove type

...
const parseLayout = (it: any) => {
    ...
    return {
        i: it._value._value.uid, x: parseInt(x), y: parseInt(y), w: parseInt(w), h: parseInt(h),
    };
};
    ...
    return (
        <ReactResizeDetector handleWidth targetRef={divRef}>
            {({ width }) => (
                <div
                  ref={divRef}
                  onContextMenu={(event) => onUnigraphContextMenu(event, data, undefined, undefined, (a: any, b: any, handleClose: any) => (
                      <MenuItem onClick={() => { handleClose(); setLayoutLocked(!layoutLocked); }}>

...
export const ConnectionWidget: React.FC = ({}) => {
    ...
    return (
        <div style={{ overflowY: 'auto', height: '100%' }}>
            ...
            <b>Graph DB Version </b>
            {' '}
            {content?.dgraph?.version}
            {' '}
            <br />
            <b>Caches in memory </b>
            {' '}
            {JSON.stringify(content?.unigraph?.cache?.names)}
            {' '}
            <br />
            <b>Total objects </b>
            {' '}
            {content?.dgraph?.objects}
            {' '}
            <br />
            <b>Total schemas </b>
            {' '}
            {content?.dgraph?.schemas}
            {' '}
            <br />
            <b>Total subscriptions </b>
            {' '}
            {content?.unigraph?.subscription?.length}
            {' '}
            <br />
            <div>
                <Button onClick={() => {
            ...
            {' '}
            <br />
            {counts.sort((a, b) => b[1] - a[1]).map((el, index) => (

import {
    Checkbox, FormControlLabel, IconButton, ListItemIcon, ListItemText, Typography,
} from '@material-ui/core';
import {
    MoreVert, OpenInNew, PlayArrow, TrendingFlat,
...
export const ViewViewDetailed: DynamicViewRenderer = ({ data, callbacks }) => {
    ...
    if (data.get('view').as('primitive')?.startsWith?.('/pages')) {
        ...
        return pages[data.get('view').as('primitive').replace('/pages/', '')]
            .constructor({ ...JSON.parse(data.get('props').as('primitive')).config, callbacks });
    }

...
export const SearchOverlayTooltip = () => {
  return (
    <div style={{ marginTop: '16px' }}>
      ...
      <Typography>
        +note / +n : create new note page and start editing
      </Typography>
  ...
  const search = React.useMemo(
    () =>
      _.debounce((query: any[]) => {
        ...
        if (query.length) {
          window.unigraph
            .getSearchResults(query, 'metadata', 3, { noPrimitives: true })
            .then(res => {
  ...
  return (
    <div>
      <InputBase
        ...
        placeholder={
          'Enter: +<type shortname> to create; ?<search query> to search; <command> to execute command'
        }
        onKeyPress={async ev => {
          if (
            ev.key === 'Enter' &&
            parsed?.type === 'quickAdder' &&
            window.unigraph.getState('registry/quickAdder').value[parsed?.key]
          ) {
            window.unigraph
              .getState('registry/quickAdder')
              .value[parsed?.key]?.adder(
      ...
      <div>
        ...
        {parsed?.type === 'quickAdder' ? (
          <Typography>{'Adding ' + parsed?.key + ' (Enter to add)'}</Typography>
        ) : (
          []
        )}
        {entries}
        ...
        {entries.length + entities.length === 0 &&
        parsed?.type !== 'command' ? (
          <SearchOverlayTooltip />
        ) : (
          []
        )}
    ...
    if (searchEnabled) {
      const listener = (event: MouseEvent) => {
        const withinBoundaries =
          overlay && event.composedPath().includes((overlay as any).current);

...
export function NoteBlock({ data }: any) {
    ...
    return (
        <div onClick={() => { window.wsnavigator(`/library/object?uid=${data.uid}&isStub=true&type=$/schema/note_block`); }} style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
...
export function OutlineComponent({
    children, collapsed, setCollapsed, isChildren, createBelow,
}: any) {
    return (
        <div style={{
        ...
        >
            ...
            <div style={{
                height: 'calc(100% + 4px)', width: '1px', backgroundColor: 'gray', position: 'absolute', left: '-12px', display: isChildren ? '' : 'none',
            }}
            />
            <FiberManualRecord style={{
                fontSize: '0.5rem', marginLeft: '8px', marginRight: '8px', ...(collapsed ? { borderRadius: '4px', color: 'lightgray', backgroundColor: 'black' } : {}),
            }}
            />
            <div style={{ flexGrow: 1 }}>
                {children}
...
export function ParentsAndReferences({ data }: any) {
    ...
    React.useEffect(() => {
        const [newPar, newRef]: any = getParentsAndReferences(data['~_value'], (data['unigraph.origin'] || []).filter((el: any) => el.uid !== data.uid));
    ...
    return (
        <div style={{ marginTop: '36px' }}>
            ...
            <DynamicObjectListView items={references} context={null} compact noDrop titleBar=" linked references" loadAll />
        </div>
    );
}

function NoteViewPageWrapper({ children, isRoot }: any) {
    return !isRoot ? children : (
...
export function DetailedNoteBlock({
    data, isChildren, callbacks, options, isCollapsed, focused,
}: any) {
                    ...
                    <div ref={childrenref} style={{ width: '100%' }}>
                        {(subentities.length || isChildren)
                            ? (
                                ...
                                >
                                    {buildGraph(subentities).map((el: any, elindex) => {

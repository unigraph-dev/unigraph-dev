...
export const parseTodoObject: (arg0: string, refs?: any[]) => ATodoList = (todoString: string, refs?: any[]) => {
    ...
    const calendarRegex = /@([^- \t\n\r"]+|"[^-"]+")(?:-([^- \t\n\r"]+|"[^-"]+"))? ?/m;
    const calendarMatches = (todoString.match(calendarRegex) || []).map((el) => (el?.startsWith('"') ? el.replace(/"/g, '') : el));
    let timeFrame;
    if (calendarMatches[1]?.length) {
        ...
        timeFrame = {
            start: {
                datetime: (!calendar[1]?.length
                    ? undefined
                    : setHours(Sugar.Date.create(calendar[1]), 0, 0, 0, 0)
                ) || getMinDate(),
                timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
            },
            end: {
                datetime: (!calendar[2]?.length
                    ? undefined
                    : setHours(Sugar.Date.create(calendar[2]), 23, 59, 59, 999)
                ) || getMaxDate(),
                timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
            },
        };

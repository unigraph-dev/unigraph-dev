import {
    Avatar, Badge, Button, Divider, ListItem, ListItemIcon, ListItemText, TextField, Typography,
...
const RSSFeedsBody: React.FC<{data: ARSSFeed[]}> = ({ data }) => {
    ...
    return (
        <div>
            ...
            <Button onClick={() => window.unigraph.runExecutable<ParserParam>(getExecutableId(rssReaderPackage, 'add-feed'), { url: newUrl })}>
                Add feed
            </Button>
            <Button onClick={() => {
                upload((f: File) => {

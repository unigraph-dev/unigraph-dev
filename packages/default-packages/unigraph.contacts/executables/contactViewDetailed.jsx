    ...
    <div>
        <Card variant="outlined" style={{ margin: '16px', padding: '16px' }}>
            <div style={{ display: 'flex', marginBottom: '8px', alignItems: 'center' }}>
                <Avatar style={{ marginRight: '16px' }} alt={`profile image of ${data?.get('name')?.as('primitive')}`} src={data?.get('profile_image')?.as('primitive')} />
            ...
            <Typography style={{ marginBottom: '8px' }}>
                ...
                <DynamicObjectListView
                    ...
                    noRemover
                    itemAdder={(uid) => {
                    unigraph.updateObject(data.uid, {
                        profiles: [{
                    ...
                    itemRemover={(uids) => { window.unigraph.deleteItemFromArray(listUid, uids, data.uid); }}
                />
            </Typography>
        </Card>

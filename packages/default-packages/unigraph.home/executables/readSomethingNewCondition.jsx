...
const feedItems = await unigraph.getQueries([`(func: uid(${inboxUid})) @normalize { _value { children { <_value[> {items: count(uid)} } } }`, 
`(func: uid(${readLaterUid})) @normalize { _value { children { <_value[> {items: count(uid)} } } }`]);
const finalItems = feedItems[0][0].items + feedItems[1][0].items;
return finalItems > 0;
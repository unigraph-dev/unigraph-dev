...
if (account?.uid) {
    ...
    const syncPage = async (pageToken) => {
        const res = await gmail.users.messages.list({userId: 'me', maxResults: 40, pageToken});
        ...
        const pageFullMsgs = await Promise.all(pageMsg.map(id => 
    ...
    if (newMsgs.length) unigraph.runExecutable('$/executable/add-email', {
        ...
        messages: newMsgs.map(el => {return {message: Buffer.from(el.data.raw, 'base64').toString(), read: (!el.data.labelIds?.includes?.('UNREAD')) || true}})
    });

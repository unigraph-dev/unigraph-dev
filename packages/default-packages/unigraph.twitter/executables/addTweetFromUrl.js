...
const getTweet = async (userAccToken, userAccTokenSecret) => {
    const [err2, res, response2] = await (new Promise((resolve, reject) => oauth.get(`https://api.twitter.com/1.1/statuses/show.json?id=${tweetId}&tweet_mode=extended`, userAccToken, userAccTokenSecret, (...ans) => resolve(ans))));

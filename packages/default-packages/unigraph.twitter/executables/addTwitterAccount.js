...
const addAccount = async (userAccToken, userAccTokenSecret) => {
    const [err, userDetails, response] = await (new Promise((resolve, reject) => oauth.get("https://api.twitter.com/1.1/account/verify_credentials.json", userAccToken, userAccTokenSecret, (...ans) => resolve(ans))));
...
oauth.getOAuthRequestToken(async (err, OAuthToken, OAuthTokenSecret, results) => {
    ...
    if (oauthResponse.query.oauth_token && oauthResponse.query.oauth_verifier) oauth.getOAuthAccessToken(

...
if (results[0].length !== 1 || results[1].length !== 1 || 
    !results[0][0].type || results[0][0].type['unigraph.id'] !== "$/schema/tag" ||
    !results[1][0].type || results[1][0].type['unigraph.id'] !== "$/schema/semantic_properties") {
        throw new Error("Wrong schema - are you sure you are supplying the correct (should have Entity type) uids?")
}

let childrenUid = results[1][0]['_value'].children.uid
let children = results[1][0]['_value'].children['_value['];

await unigraph.updateObject(childrenUid, {'_value[': {
